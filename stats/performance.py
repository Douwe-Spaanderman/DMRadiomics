...
def extract_performance_measurements(
    input_root: str = "../data/results", output_root: str = "../data/results"
) -> None:
    """
    ...
    for experiment in os.listdir(input_root):
        ...
        print(
            f"Found experiment: {experiment} with label: {label}, sequence: {sequence} and center: {center}"
        )
        ...
        data[experiment] = {
            ...
            "sequence": sequence,
            **{
                measurement: performance["Statistics"].get(measurement + " 95%:", None)
        ...
        for label in set(item["label"] for item in data.values()):
            for sequence in set(item["sequence"] for item in data.values()):
                ...
                d = {
                    ...
                    if value["label"] == label and value["sequence"] == sequence
                }
                d = pd.DataFrame(d).T
                ...
                # Clean sheet name
                sheet_name = (
                    f"{label}_{sequence}"[:31].replace(":", "_").replace("/", "_")
                )
