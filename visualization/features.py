...
def plot_significant_features(
    data: Dict[str, str],
    statistics: str = "Mann-Whitney",
    output_path: str = "feature_plots",
    p_thresh: float = 0.05,
    annotate_thresh: float = 9e-5,
) -> None:
    """
    ...
    unique_centers = [
        meta["metadata"].get("internalcenter", "None") for meta in data.values()
    ]
    if center_marker_map.keys() != set(unique_centers):
        missing_centers = set(center_marker_map.keys()) - set(unique_centers)
        print(
            f"Warning: Not all experiments have been run, and {missing_centers} is/are missing."
        )
    ...
    feature_significance_map = (
        feature_data.groupby("Label")[statistics].min() < p_thresh
    )
    ...
    group_color_map = {
        group: palette[i] for i, group in enumerate(sorted(set(groups.values())))
    }
    ...
    for _, row in feature_data[feature_data[statistics] < annotate_thresh].iterrows():
        annotations.append(
            plt.text(row["x"], row[statistics], row["name"], ha="center", va="bottom")
        )
    ...
    plt.xticks(
        xtick_positions, xtick_labels, rotation=45, ha="right", rotation_mode="anchor"
    )
    ...
    legend_elements.append(
        Patch(label=r"$\bf{Center:}$", facecolor="none", edgecolor="none")
    )
...
def save_features_by_center(
    data: Dict[str, str],
    output_path: str = "significant_features.xlsx",
    statistics: str = "Mann-Whitney",
    ...
    data = reduce(
        lambda left, right: pd.merge(
            left, right, on=["Label", "group", "name"], how="inner"
        ),
        feature_data,
    )
    ...
    sig_counts = sig_matrix.sum(
        axis=1
    )  # Count how many centers each feature is significant in
