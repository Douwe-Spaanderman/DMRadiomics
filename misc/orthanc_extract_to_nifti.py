...
def convert_dicom_to_nifti(dicom_dir, output_file):
    ...
    for f in os.listdir(os.path.dirname(output_file)):
        if f.startswith("image") and f.endswith(".nii.gz"):
            os.rename(
                os.path.join(os.path.dirname(output_file), f),
                output_file
            )
...
def convert_seg_to_nifti(dicom_seg_dir, output_file):
    ...
    subprocess.run([
        "segimage2itkimage",
        "--inputDICOM", os.path.join(dicom_seg_dir, seg_files[0]),
        "--outputDirectory", os.path.dirname(output_file),
        "--outputType", "nifti",
        "--prefix", "mask",  # Output will be mask.nii.gz
    ], check=True)
...
def find_metadata_match(metadata, study_folder):
    ...
    patient_meta["Segmented sequence"] = patient_meta["series_instance_uid"] == meta["referenced_series_uid"]
    ...
    image_name = patient_meta.loc[patient_meta["Segmented sequence"], "Sequence Name"].values[0]
    patient_meta = patient_meta[patient_meta["session"] == patient_meta.loc[patient_meta["Segmented sequence"], "session"].values[0]]
...
def process_patient(patient_dir, metadata):
    ...
    for study_folder in os.listdir(patient_dir):
        ...
        print(f"✅ Saved NIfTIs for {patient_name} in:\n  {image_nifti}\n  {mask_nifti}")
...
def main():
    ...
    metadata = pd.read_csv(
        os.path.join(INPUT_ROOT, "metadata_analyzed.csv")
    )
