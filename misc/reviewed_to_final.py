...
def postprocess_patient(patient_input_dir, patient_output_dir):
    ...
    for image_name, review_info in review.items():
        ...
        if base_name in used_output_names:
            raise RuntimeError(
                f"Conflict: Multiple reviewed sequences map to {base_name} in {patient_input_dir}"
            )
        ...
        original_mask_name = image_name.replace(
            ".nii.gz", f"-{chosen_mask_type}-mask.nii.gz"
        )
