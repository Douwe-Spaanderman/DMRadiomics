...
rigid_custom = {"MaximumNumberOfIterations": ["500"], "NumberOfResolutions": ["4"]}

affine_custom = {"MaximumNumberOfIterations": ["1000"], "ImageSampler": ["Random"]}
...
def get_sequence_variations(patient_dir):
    ...
    nii_files = [
        f for f in nii_files if not any(x in f for x in ["-mask", "-registered-mask"])
    ]
...
def find_reference_sequence(patient_dir, sequences):
    ...
    for seq_name in sequences:
        ...
        mask_path_v2 = os.path.join(
            patient_dir, f"{seq_name.split('-')[0]}-mask.nii.gz"
        )
    ...
    try:
        ...
        try:
            ...
            rigid_transformed_mask = itk.transformix_filter(
                moving_mask, rigid_transform_params, log_to_console=False
            )
        ...
        try:
            ...
            affine_transformed_mask = itk.transformix_filter(
                moving_mask, affine_transform_params, log_to_console=False
            )
