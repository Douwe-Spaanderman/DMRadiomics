...
def extract_performance_measurements(
    data: Dict[str, dict], DeLong: Dict[str, float]
) -> Dict[str, dict]:
    """
    ...
    for experiment, content in data.items():
        ...
        final[experiment] = {
            **meta,
            **{
                measurement: performance.get(measurement + " 95%:", None)
...
def run_analyze(experiment_root: str, output_root: str) -> None:
    ...
    for experiment in experiments:
        ...
        roc_data = read_roc_data(
            os.path.join(experiment_path, "Evaluation", "ROC_all_0.csv")
        )

        # Load probability data for statistical test
        ranked_posterior = read_posterior_data(
            os.path.join(experiment_path, "Evaluation", "RankedPosteriors_all_0.csv")
        )

        # Load feature importance data if available
        features_data = read_feature_data(
            os.path.join(
                experiment_path, "Evaluation", "StatisticalTestFeatures_all_0.csv"
            )
        )
    ...
    for combination in unique_combinations:
        ...
        feature_importance = {
            name: data
            for name, data in matching_experiments.items()
            if data["metadata"].get("externalcenter", "") == "None"
            and data["metadata"].get("internalcenter", "") != "All"
            and data["metadata"].get("Clinical", "") == "None"
        }
        if feature_importance:
            if len(feature_importance) > 3:
                print(
                    "Warning: More than 3 experiments found for feature importance, expected 3."
                )

            save_features_by_center(
                data=feature_importance,
                output_path=os.path.join(combination_path, "features_by_center.xlsx"),
            )
            plot_significant_features(
                data=feature_importance,
                output_path=os.path.join(combination_path, "features_importance.png"),
            )
        ...
        plot_ROC(
            data=ROC_data,
            output_path=os.path.join(combination_path, "ROC_Imaging_and_Location.png"),
        )
        ROC_data = {
            name: data
            for name, data in matching_experiments.items()
        ...
        plot_ROC(
            data=ROC_data,
            output_path=os.path.join(combination_path, "ROC_Imaging_and_Clinical.png"),
        )
        ROC_data = {
            name: data
            for name, data in matching_experiments.items()
        ...
        plot_ROC(
            data=ROC_data,
            output_path=os.path.join(combination_path, "ROC_Clinical_only.png"),
        )
        ...
        unique_centers = set(
            [data["metadata"]["externalcenter"] for data in external_data.values()]
        )
        DeLong_results = {}
        ranked_posteriors = []
        for center in unique_centers:
            baseline = [
                ...
                if data["metadata"].get("Clinical", "") == "None"
                and data["metadata"].get("externalcenter", "") == center
            ]
            ...
            for key, roc in DeLong_rocB.items():
                DeLong_results[key] = calculate_DeLong(
                    baseline_ranked_posterior, roc["ranked_posterior"]
                )
        ...
        ranked_posteriors = ranked_posteriors.merge(
            metadata, left_on="PatientID", right_on="Patient", how="left"
        )
        robustness = []
        for subgroup, values in subgroups.items():
            for value in values:
                ...
                if subset["TrueLabel"].nunique() < 2:
                    print(
                        f"Skipping subgroup {subgroup} with value {value}: not enough unique labels."
                    )
                    continue
        ...
        performance_data.update(
            extract_performance_measurements(matching_experiments, DeLong_results)
        )
    ...
    performance_data = performance_data.sort_values(
        ["label", "externalcenter", "internalcenter", "seq", "addseq", "Clinical"]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Analyze all WORC experiments for desmoid-type fibromatosis"
    )
    parser.add_argument(
        "-e",
        "--experiment_path",
